...
POINTERSIZE randomseed;                     /* Current random number seed. */
...
/*  triangles are four-byte-aligned and that the `POINTERSIZE' type is     */
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((POINTERSIZE) (ptr) & (POINTERSIZE) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((POINTERSIZE) (ptr) ^ (POINTERSIZE) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((POINTERSIZE) (otri).tri | (POINTERSIZE) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((POINTERSIZE) (otri).tri[6] | (POINTERSIZE) 2l)
...
#define infected(otri)                                                        \
  (((POINTERSIZE) (otri).tri[6] & (POINTERSIZE) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((POINTERSIZE) (sptr) & (POINTERSIZE) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((POINTERSIZE) (sptr) & ~ (POINTERSIZE) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((POINTERSIZE) (osub).ss | (POINTERSIZE) (osub).ssorient)
...

{
    ...
    if (printvertex == (vertex)NULL)
        ...
        printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
            (t->orient + 1) % 3 + 3, (POINTERSIZE)printvertex,
    ...
    if (printvertex == (vertex)NULL)
        ...
        printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
            (t->orient + 2) % 3 + 3, (POINTERSIZE)printvertex,
            printvertex[0], printvertex[1]);
    apex(*t, printvertex);
    if (printvertex == (vertex)NULL)
        printf("    Apex  [%d] = NULL\n", t->orient + 3);
        ...
        printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
            t->orient + 3, (POINTERSIZE)printvertex,
            printvertex[0], printvertex[1]);

    if (b->usesegments) {
        sdecode(t->tri[6], printsh);
        if (printsh.ss != m->dummysub) {
            printf("    [6] = x%lx  %d\n", (POINTERSIZE)printsh.ss,
                printsh.ssorient);
        }
...

{
    ...
    printf("subsegment x%lx with orientation %d and mark %d:\n",
        (POINTERSIZE)s->ss, s->ssorient, mark(*s));
    ...
    if (printvertex == (vertex)NULL)
        ...
        printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
            2 + s->ssorient, (POINTERSIZE)printvertex,
            printvertex[0], printvertex[1]);
    sdest(*s, printvertex);
    if (printvertex == (vertex)NULL)
        printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
        ...
        printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
            3 - s->ssorient, (POINTERSIZE)printvertex,
            printvertex[0], printvertex[1]);
    ...
    if (printvertex == (vertex)NULL)
        ...
        printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
            4 + s->ssorient, (POINTERSIZE)printvertex,
            printvertex[0], printvertex[1]);
    ...
    if (printvertex == (vertex)NULL)
        ...
        printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
            5 - s->ssorient, (POINTERSIZE)printvertex,
            printvertex[0], printvertex[1]);
}
...

{
    ...
    alignptr = (POINTERSIZE)(pool->nowblock + 1);
    /* Align the item on an `alignbytes'-byte boundary. */
    pool->nextitem = (VOID*)
        (alignptr + (POINTERSIZE)pool->alignbytes -
            (alignptr % (POINTERSIZE)pool->alignbytes));
...

{
    ...
    alignptr = (POINTERSIZE)(pool->pathblock + 1);
...

{
    ...
    alignptr = (POINTERSIZE)m->dummytribase;
    m->dummytri = (triangle*)
        (alignptr + (POINTERSIZE)m->triangles.alignbytes -
            (alignptr % (POINTERSIZE)m->triangles.alignbytes));
    ...
    if (b->usesegments) {
        ...
        alignptr = (POINTERSIZE)m->dummysubbase;
        m->dummysub = (subseg*)
            (alignptr + (POINTERSIZE)m->subsegs.alignbytes -
                (alignptr % (POINTERSIZE)m->subsegs.alignbytes));
...

{
    ...
    alignptr = (POINTERSIZE)(getblock + 1);
    foundvertex = (char*)(alignptr + (POINTERSIZE)m->vertices.alignbytes -
        (alignptr % (POINTERSIZE)m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
POINTERSIZE randomnation(unsigned int choices)
#else /* not ANSI_DECLARATORS */
POINTERSIZE randomnation(choices)
...

{
    ...
    while (totalsamplesleft > 0) {
        ...
        alignptr = (POINTERSIZE)(sampleblock + 1);
        firsttri = (char*)(alignptr +
            (POINTERSIZE)m->triangles.alignbytes -
            (alignptr %
                (POINTERSIZE)m->triangles.alignbytes));
