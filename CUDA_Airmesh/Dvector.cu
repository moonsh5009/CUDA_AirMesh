...
__global__ void vectorSum_kernel(uint* X, size_t size, uint* result) {
...
void getDvectorSum(const Dvector<uint>& X, uint* deviceResult) {
	uint length = X.size();
	vectorSum_kernel << <divup(length, MAX_BLOCKSIZE << 1u), MAX_BLOCKSIZE, MAX_BLOCKSIZE * sizeof(uint) >> > (
		X._list, length, deviceResult);
	CUDA_CHECK(cudaPeekAtLastError());
}